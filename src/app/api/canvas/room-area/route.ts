...
interface RoomAreaResults {
  status: string;
  link_room_area_processing: string;
}
...
async function addRedLineToImage(imageUrl: string): Promise<Buffer> {
  try {
    ...
    // Calculate line position for top-left corner - smaller and more subtle
...
export async function POST(request: Request) {
  try {
    ...
    const apiResponse = await fetch('https://1e3f-2a02-4780-28-6eb6-00-1.ngrok-free.app/arcus/room_area', {
      method: 'POST',
      headers: {
    ...
    const detectionResults: RoomAreaResults = await apiResponse.json();

    // 5. Update the project with the new canvas data
    const updatedCanvasData: CanvasData = {
      ...canvasData,
      pages: canvasData.pages || [],
      room_area_processing: [...(canvasData.room_area_processing || [''])]
    };

    // Initialize array with empty strings if it doesn't exist
    if (!updatedCanvasData.room_area_processing) {
      updatedCanvasData.room_area_processing = [];
    }

    // Ensure array has enough slots
    while (updatedCanvasData.room_area_processing.length <= currentPage) {
      updatedCanvasData.room_area_processing.push('');
    }

    // Store detection URL at the correct index
    updatedCanvasData.room_area_processing[currentPage] = detectionResults.link_room_area_processing || '';
    ...
    return NextResponse.json({
      success: true,
      modifiedImageUrl: modifiedCloudinaryUrl, // Include this to verify the red line
