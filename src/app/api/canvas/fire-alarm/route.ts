...
export async function POST(request: Request) {
  try {
    ...
    const detectionResults: FireAlarmResults = await apiResponse.json();
    ...
    while (updatedCanvasData.fire_alarm_processing.length <= currentPage) {
      updatedCanvasData.fire_alarm_processing.push('');
    }

    // Store detection URL at the correct index
    updatedCanvasData.fire_alarm_processing[currentPage] = detectionResults.link_exclusion_Zones_processing || '';
